...
in the \texttt{trust} package by Charles J. Geyer, reproduced here
unchanged because it documents the exact algorithm \texttt{RcppTrust}
ports to thread-safe C++ (RcppArmadillo for the linear algebra,
\texttt{boost::math::tools::toms748\_solve} in place of
\texttt{stats::uniroot()} for the one-dimensional root-find in
Section~\ref{sec:subproblem}). Every case split, formula, and ordering
decision below is implemented identically in the C++ core; only the
numerical software used to carry it out has changed.
...
is concerned) one solution is just as good as another. In particular, the
sign of $\tau$ chosen by a given eigendecomposition routine at an exact
eigenvalue tie is itself part of this non-uniqueness: RcppTrust's use of
RcppArmadillo/LAPACK's \texttt{eig\_sym} rather than R's \texttt{eigen()}
can pick the opposite sign than the R implementation does in the
hard-hard case, giving a mirror-image (but equally valid) solution.
...
nearly linear. RcppTrust finds this zero with
\texttt{boost::math::tools::toms748\_solve} (a bracketing,
derivative-free root-finder, like R's \texttt{uniroot()}) rather than
\texttt{uniroot()} itself, using the same bracket derived below.
